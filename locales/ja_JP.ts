...
PublishedFileIDs are needed to retrieve mods from Steam!</source>
        <translation>これは通常、Steam WebAPI キーが無効または欠落していること、または Steam WebAPI への接続の問題が原因で発生します。
Steam から MOD を取得するには PublishedFileID が必要です。</translation>
    </message>
    <message>
...
A report of discrepancies is generated. You will be prompted for these paths in order:
	...
	2) Select input B</source>
        <translation>- これにより、2 つの Steam DB 間の依存関係データの正確な比較が生成されます。
不一致のレポートが生成されます。これらのパスを順番に入力するよう求められます。

	1) 入力 A を選択します
	2) 入力Bを選択します</translation>
    </message>
    <message>
...
The resultant database, C, is saved to a user-specified path. You will be prompted for these paths in order:
	...
	3) Select output C (resultant db)</source>
        <translation>- これにより、結果として得られるデータベースに対して、A のキー/値が B のキー/値で効果的に再帰的に上書きされます。
- 例外は再帰的に更新されません。代わりに、B のキーで完全に上書きされます。
- 以下の例外が発生します。

	{DB_BUILDER_RECURSE_EXCEPTIONS}

結果として得られるデータベース C は、ユーザーが指定したパスに保存されます。これらのパスを順番に入力するよう求められます。

	1) 入力 A (更新されるデータベース) を選択します
	2) 入力 B (更新元) を選択します。
	3) 出力 C (結果の db) を選択します。</translation>
    </message>
    <message>
...
{url}</source>
        <translation>アクティブな MOD リストのレポートを Rentry.co にアップロードしました。 URL がクリップボードにコピーされました:
...
The updated versions won't appear until you refresh. Refresh now?</source>
        <translation>{count} 個の MOD が自動更新されました。



更新されるまで、更新されたバージョンは表示されません。今すぐリフレッシュしますか？ {summary}</translation>
    </message>
    <message>
