...
PublishedFileIDs are needed to retrieve mods from Steam!</source>
        <translation>这通常是由于 Steam WebAPI 密钥无效/丢失，或者 Steam WebAPI 的连接问题造成的。
从 Steam 检索模组需要 PublishedFileID！</translation>
    </message>
    <message>
...
A report of discrepancies is generated. You will be prompted for these paths in order:
	...
	2) Select input B</source>
        <translation>- 这将产生 2 个 Steam DB 之间依赖数据的准确比较。
生成差异报告。系统将按顺序提示您输入这些路径：
...
The resultant database, C, is saved to a user-specified path. You will be prompted for these paths in order:
	...
	3) Select output C (resultant db)</source>
        <translation>- 这将有效地用 B 的键/值递归地覆盖结果数据库中的 A 的键/值。
- 异常不会被递归更新。相反，它们将被 B 的密钥完全覆盖。
- 以下情况除外：

	{DB_BUILDER_RECURSE_EXCEPTIONS}

生成的数据库 C 将保存到用户指定的路径。系统将按顺序提示您输入这些路径：

	1）选择输入A（待更新的db）
	2）选择输入B（更新源）
	3) 选择输出C（结果db）</translation>
    </message>
    <message>
...
{url}</source>
        <translation>已将活跃模组列表报告上传至 Rentry.co！ URL 已复制到您的剪贴板：
...
The updated versions won't appear until you refresh. Refresh now?</source>
        <translation>{count} 个模组已自动更新。



刷新后才会显示更新的版本。现在刷新？ {summary}</translation>
    </message>
    <message>
