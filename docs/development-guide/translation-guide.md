...
- **`.ts` files**: Source translation files (XML format) that translators edit
  - **`.qm` files**: Compiled binary translation files used by the application. These are generated by `just i18n-compile` (and by CI) and **are committed** to the repository.
...
before the application can load them. The compiled `.qm` files **are committed**
to the repository so users can use translations directly from a checkout or release
without recompiling. During development they are (re)generated by `just i18n-compile`
or `just dev-setup`.
...
| `zh_TW` | 正體中文 (Traditional Chinese) | Complete |
...
5. **Submit code**: Commit both `.ts` and `.qm` files (the `.qm` files are also generated automatically by `just i18n-compile` and CI, and they are committed to the repository)
...
7. **Submit code**: Commit both `.ts` and `.qm` files (the `.qm` files are also generated automatically by `just i18n-compile` and CI, and they are committed to the repository)
...
3. Register the new language in the language controller:
   ...
   Open the file `app/controllers/language_controller.py` and find the `language_map` dictionary in the `populate_languages_combobox` method, add your language:
    ...
    language_map = {
        ...
        "zh_TW": "正體中文",
        "ko_KR": "한국어",  # Add new language entry
    }
    ```
...
1. **Commit your changes**:
    ...
    ```bash
    # Add translation files (.ts source files and the compiled .qm files, both are committed)
    ...
    **Note**:
    1. Compilation generates corresponding `.qm` files in the `locales/` directory.
    2. `.qm` files **are committed** to the repository alongside the `.ts` files, so users can use translations directly from a checkout or release without recompiling. They are (re)generated by `just i18n-compile`, `just dev-setup`, and CI.
    3. Always run `just i18n-update` then `just i18n-compile`, and commit both the `.ts` and `.qm` changes.
