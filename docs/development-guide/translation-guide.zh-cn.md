...
1. **编译翻译文件**：
   ...
   **注意**：
   ...
   2. 这些文件也会通过 GitHub Actions 生成，因而在某些构建场景下可能被视为“构建产物”；在那种情况下通常不提交 `.qm` 文件。
   3. `just dev-setup` 和 `just build` 会自动生成它们。
   4. 在本项目中，`.qm` 文件也会提交到版本控制中，以便用户在下载后无需额外编译步骤即可直接使用翻译功能。（仍需由提交者生成并提交；未来将自动化）
