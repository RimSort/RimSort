...
- RimSort 还使用 **rimsort_steam**，一个自定义的 Steamworks API C++ 封装库，位于 `libs/rimsort_steam/`。所有平台的预编译二进制文件已提交到 `libs/` 目录，大多数开发者无需从源码构建。
...
- **steamfiles** 依赖因特殊原因无法通过 requirements.txt 直接安装

  - 参阅其章节了解配置方法，或使用 `distribute.py` 自动安装。默认情况下该脚本会直接构建 RimSort，但可通过参数配置启用/禁用构建等步骤。使用 `python distribute.py --help` 查看详细说明
...
### 使用 rimsort_steam 二进制文件

RimSort 使用 **rimsort_steam**，一个轻量级的 [Steamworks API](https://partner.steamgames.com/doc/api) C++ 封装库。所有平台的预编译二进制文件和 Steam SDK 可再分发组件已提交到 `libs/` 目录。大多数开发者只需克隆仓库即可，无需额外操作。

### 从源码构建 rimsort_steam

这是 _**可选**_ 步骤。你 _**无需**_ 执行此操作——预编译的二进制文件已提交到 `libs/` 目录。仅在修改原生封装库或更新 Steamworks SDK 时才需要重新构建。

从源码构建步骤：

1. 下载并解压 [Steamworks SDK](https://partner.steamgames.com/)。
2. 设置 `STEAMWORKS_SDK_PATH` 环境变量指向解压后的 SDK 根目录。
3. 在 `libs/rimsort_steam/` 目录下运行 `make`。

平台要求：

- 在 Linux 上，你需要安装 `g++`。Ubuntu 上可直接使用，无需额外配置。
- 在 macOS 上，你需要安装 Xcode 命令行工具（`xcode-select --install`）。
- 在 Windows 上，使用提供的 `build_windows.bat`（需要 MSVC / Visual Studio 构建工具）。

未经维护者同意，请勿尝试提交/发起更新这些二进制文件的 PR 请求——此类请求将不予批准。
