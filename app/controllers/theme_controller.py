...
class Themes:
    ...
    def get_theme_storage_location_path(self) -> Path:
        """
        Get the theme folder in the theme storage folder.
        """
        storage_folder_path = AppInfo().theme_storage_folder
        logger.warning(f"theme path is {storage_folder_path}")
    ...
    def validate_theme(self) -> None:
        ...
        if self.theme_name not in supported_themes:
            ...
            logger.warning(
                f"Stylesheet file is Missing or Invalid in '{THEME_FOLDERS}', "
                f"Applying '{self.theme_name}' Theme."
            )
    ...
    def style_sheet(self) -> str:
        ...
        if theme_data_folder_path.exists():
            ...
            logger.error(
                f"Stylesheet file including Default Theme not found in '{THEME_FOLDERS}', "
                f"Applying '{self.theme_name}' Theme"
            )
            return ""
    ...
    def theme_icon(self) -> str:
        ...
        if theme_data_folder_path.exists():
            ...
            logger.error(
                f"Icon folder including Default Icons not found in '{THEME_FOLDERS}', "
                f"Applying '{self.theme_name}' Icons"
            )
            return ""
            ...
            for folder in theme_path.iterdir():
                if folder.is_dir():
                    ...
                    if stylesheet_path.exists():
                        ...
                        logger.warning(
                            f"Skipping folder '{folder.name}' in `{theme_path}', "
                            f"As it does not contain a valid stylesheet."
                        )
