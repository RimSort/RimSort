...
def do_topo_sort(
    ...
    Returns:
        ...
        (and then by alphabetical name within each level).
    """
    logger.info(f"Initializing topological sort for {len(dependency_graph)} mods.")

    # Single instance of the MetadataManager for retrieving mod info.
    ...
    for level in topological_levels:
        ...
        active_mods_in_level = [
            active_pid_to_uuid[package_id] for package_id in level
            if package_id in active_pid_to_uuid
        ]
        # Sort those mods by their (human-readable) name for a stable, alphabetical order.
        active_mods_in_level.sort(
            key=lambda mod_uuid: metadata_manager.internal_local_metadata[mod_uuid]["name"]
        )
        # Extend our master list in the sorted order.
